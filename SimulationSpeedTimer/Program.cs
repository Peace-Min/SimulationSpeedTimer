...
namespace SimulationSpeedTimer
{
    ...
    internal class MainTest
    {
        static void Main(string[] args)
        {
            ...
            // 실제 앱 실행 전 스트레스 테스트 수행
            SimulationSpeedTimer.Tests.MockTest.RunStressTest();
            ...
            if (System.IO.File.Exists(dbPath))
            {
                ...
                {
                    controller.AddService(cfg.Item1, cfg.Item2);
                }
            }
            else
            {
                ...
                if (Console.KeyAvailable)
                {
                    ...
                    switch (key)
                    {
                        case ConsoleKey.S:
                            ...
                            try
                            {
                                controller.Start(queryInterval: 1.0); // start signature matched
                        ...
                        case ConsoleKey.P:
                            Console.WriteLine("\n[User Input] Pause (Not Supported in Controller, Timer paused only)");
                            // controller.Pause(); // Removed
                            SimulationTimer.Pause(); // Assuming SimulationTimer has Pause, usually Controller manages timer but here tests might be different
                            // As per user request, pause logic is removed from Controller.
                            // If MainTest wants to simulate pause, it should stop sending time or use SimulationTimer.Pause
